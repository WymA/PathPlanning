...
void TMOEAD::update_problem(CIndividual &indiv, int id)
{
	for( int i = 0 ; i < m_neighborhood_size ; i++ ){
		...
		double f1, f2 ;
		f1 = scalar_func(population[k].indiv.y_obj, population[k].namda, reference_point) ;
		f2 = scalar_func(indiv.y_obj, population[k].namda, reference_point) ;
...
void TMOEAD::get_best_obj( int i_obj, vector<int> &path ) 
{
	...
	for( int i = 0; i < m_population_size; ++i ){
		...
		if ( population[i].indiv.rank == 1 ){
			...
			curBestiObjVal = population[i].indiv.y_obj[ i_obj ] ;
			curBestiObjIdx = i ;
			break;
		}
	}

	for( int i = curBestiObjIdx + 1; i < m_population_size; ++i) {

		if ( population[i].indiv.rank == 1 && population[i].indiv.y_obj[ i_obj ] < curBestiObjVal ) {
	...
	path = population[curBestiObjIdx].indiv.x_var ;
}
