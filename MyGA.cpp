...
void MyGA::InitInd(individual &ind)
{
	...
	if(j < 3){//只有起点和终点
		...
		ind.xPath.push_back(out_num);//加入终点
	}
}
...
void MyGA::GetPopBestObj(int iObj, vector<int> &path)
{
	...
	for( int i = 0; i < popSize; ++i ){
		...
		if ( parent_pop.ind[i].rank == 1 ){
			...
			cur_best_obj_value = parent_pop.ind[ i ].obj[ iObj ] ;
	...
	for( int i = cur_best_obj_index + 1; i < popSize; ++i) {
		...
		if ( parent_pop.ind[i].rank == 1 && parent_pop.ind[i].obj[ iObj ] < cur_best_obj_value ) {
	...
	path = parent_pop.ind[cur_best_obj_index].xPath;
...
void MyGA::GenMutationInd(individual &ind)
{
	...
	if( p < Pm && ind.xPath.size() > 2){
		...
		if(bSeries != 2){
			...
			int newVal = rand()%(out_num+1);
			int x = newVal%chart_width;
			int y = newVal/chart_width;
...
void MyGA::EvaluateInd(individual &ind, int index )
{
	...
	for (int j = 1; j < n-1; j++){
		...
		x2 = ind.xPath[j] % chart_width;
		y2 = ind.xPath[j] / chart_width;
		x3 = ind.xPath[j+1] % chart_width;
		y3 = ind.xPath[j+1] / chart_width;
	...
	for(int j = 0; j < n-1; ++j){
		...
		double len = 0.0;
		double L = 0.0;
		x1 = ind.xPath[j]%chart_width;
		y1 = ind.xPath[j]/chart_width;
		vector<int> vecChart;
		...
		for(int i = 0; i < vecChart.size(); ++i){
			...
			if(check(ind,((y2-1)*chart_width+x2)) && check(ind,(y2*chart_width+x2-1)))
				m += 1;
			else if(check(ind,((y2+1)*chart_width+x2)) && check(ind,(y2*chart_width+x2-1)))
				m += 1;
			else if(check(ind,((y2+1)*chart_width+x2)) && check(ind,(y2*chart_width+x2+1)))
				m += 1;
			else if(check(ind,((y2-1)*chart_width+x2)) && check(ind,(y2*chart_width+x2+1)))
	...
	for ( int i = 0 ; i < n - 1 ; i++ ){
		...
		x1 = ind.xPath[i] % chart_width;
		y1 = ind.xPath[i] / chart_width;
		x2 = ind.xPath[i+1] % chart_width;
		y2 = ind.xPath[i+1] / chart_width;
